...
텔레그램에서 미국 상장기업을 조회하고, 재무 데이터와 품질 점수, 로컬 LLM 기반 전문가별 해석을 보여주는 개인용 봇입니다.
...
DAILY_NEWS_LLM_TIMEOUT_SECONDS=300
WATCHLIST_MAX_ITEMS=10
WATCHLIST_LIMIT_EXEMPT_CHAT_IDS=
```

봇 토큰은 서버에서만 사용합니다. 텔레그램 사용자에게 공유하지 않습니다.
`LOCAL_LLM_MODEL`을 비워두면 전문가별 LLM 해석은 생략됩니다. Ollama를 쓴다면 예를 들어 `LOCAL_LLM_MODEL=qwen3.6:27b`처럼 로컬에 설치된 모델명을 넣습니다.
...
명령어 대신 짧은 문장으로도 일부 기능을 사용할 수 있습니다.
